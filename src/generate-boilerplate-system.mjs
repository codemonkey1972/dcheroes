...
class SystemGenerator {
   ...
   * Rename files.
   *
   * Rename files that had dcheroes in their name, such as
   * css/dcheroes.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*dcheroes*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('dcheroes', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-dcheroes-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    glob('*', {ignore: 'node_modules/**' }).then(files => {
      ...
      // Replace dcheroes name mentions in files.
      generator.replaceFileContents();
      // Rename files that had dcheroes in their name.
